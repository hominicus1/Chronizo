...
function base(g){
 ...
 [...Object.values(locations).filter(l=>l.x<1100),...customLocations].forEach(l=>{const pin=el('g',{class:'map-pin'});append(pin,el('circle',{cx:l.x,cy:l.mapY,r:4}),el('text',{x:l.x+8,y:l.mapY-6},l.name));pin.onclick=ev=>{ev.stopPropagation();show({title:l.name,place:l.zone||'Własna lokalizacja',tags:['Lokalizacja']},'LOKALIZACJA')};p.append(pin)});
...
function setZoom(next,cx=720,cy=450){next=Math.max(.55,Math.min(2.6,next));const ratio=next/zoom;panX=cx-720-(cx-720-panX)*ratio;panY=cy-450-(cy-450-panY)*ratio;zoom=next;render()}
svg.addEventListener('wheel',e=>{e.preventDefault();const p=pointerPosition(e);setZoom(zoom*(e.deltaY<0?1.12:.89),p.x,p.y)},{passive:false});
svg.addEventListener('pointerdown',e=>{if(placingLocation)return;svg.setPointerCapture(e.pointerId);pointers.set(e.pointerId,pointerPosition(e))});
...
['pointerup','pointercancel'].forEach(type=>svg.addEventListener(type,e=>pointers.delete(e.pointerId)));
...
svg.addEventListener('click',e=>{if(!placingLocation)return;const pt=svg.createSVGPoint();pt.x=e.clientX;pt.y=e.clientY;const local=pt.matrixTransform(svg.getScreenCTM().inverse());const name=prompt('Nazwa lokalizacji:');placingLocation=false;$('#map-wrap').classList.remove('location-mode');$('#location-mode-banner')?.remove();if(!name?.trim())return;customLocations.push({name:name.trim(),x:Math.round(local.x),mapY:Math.round(local.y),zone:'Własna lokalizacja · '+worlds.find(w=>w.id===active).name});render();show({title:name.trim(),place:'Własna lokalizacja',tags:['Lokalizacja'],copy:'Lokalizacja została dodana do aktywnego świata.'},'LOKALIZACJA')});
