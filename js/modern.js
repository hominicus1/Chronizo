import {createLocation,getLocationPath,nearestParent,visibleAtZoom,LOCATION_LEVELS} from './locations.js?v=4.0.0-alpha.20';
...
function base(g){
 ...
 p.append(el('image',{href:customTexture||'assets/world-map.svg?v=4.0.0-alpha.20',x:0,y:0,width:1000,height:500,transform:'matrix(.77 .04 -.14 .36 250 620)',class:'map-texture'}));
 g.append(p);
...
$('#btn-place-map').onclick=()=>{if(!selectedPlace)return;const detailed=['street','building','room'].includes(selectedPlace.type),level=detailed?18:14,{lat,lon}=selectedPlace,url=`https://www.openstreetmap.org/?mlat=${encodeURIComponent(lat)}&mlon=${encodeURIComponent(lon)}#map=${level}/${lat}/${lon}`,tab=window.open(url,'_blank','noopener');if(!tab)window.location.href=url};
