...
for(const id of ['world-tabs','story-map','tag-filters','inspector','add-dialog','map-texture-input','build-version','btn-place-map'])assert.match(html,new RegExp('id=["\\\']'+id+'["\\\']'));
...
assert.match(html,/id="btn-place-map"[^>]+href="https:\/\/www\.openstreetmap\.org\//,'working external detailed map link missing');
assert.ok(js.includes("button.href=`https://www.openstreetmap.org/"),'dynamic detailed map link missing');
...
assert.match(html,/α21/,'visible build version missing');
