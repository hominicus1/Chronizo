...
for(const id of ['world-tabs','story-map','tag-filters','inspector','add-dialog','map-texture-input','build-version','btn-place-map','place-map-dialog','place-map-frame'])assert.match(html,new RegExp('id=["\\\']'+id+'["\\\']'));
...
assert.ok(js.includes('zoom>=10'),'detail map zoom threshold missing');
...
assert.match(html,/α17/,'visible build version missing');
