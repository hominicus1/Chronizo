import {createLocation,getLocationPath,nearestParent,visibleAtZoom,LOCATION_LEVELS} from './locations.js?v=4.0.0-alpha.17';
...
const labelStyle=(size,outline=1.8)=>`font-size:${size/zoom}px;stroke-width:${outline/zoom}px`;
...
function updatePlaceMapButton(){const button=$('#btn-place-map');if(button)button.hidden=!(selectedPlace&&zoom>=10)}
...
function event(g,e){const color=e.char?people[e.char].color:worlds.find(w=>w.id===active).color,n=el('g',{class:'event-node event-group '+(matches(e)?'':'muted'),style:'color:'+color});append(n,el('circle',{cx:e.x,cy:e.y,r:Math.max(1.5,5/zoom)}),el('text',{x:e.x+10/zoom,y:e.y-7/zoom,style:labelStyle(10,2)},e.title));n.onclick=()=>{selectPlace(e);show(e,e.target?'PRZEJŚCIE MIĘDZY ŚWIATAMI':'WYDARZENIE');liftMapTo(e)};g.append(n)}
